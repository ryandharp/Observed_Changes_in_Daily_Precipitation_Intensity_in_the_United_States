...
def bootstrap_pdf_creator(region_num):

    # looping over first half years
    region_first_half_pdf = np.array([])

    for yr in np.arange(1951, 1981, 2):
        fpath = '/home/rdh0715/NCA_region_bootstrapping/region_' + str(region_num) + '/first_half/' + str(yr) + '_' + str(yr+1) + '/'
        ...
        for block_num in rand_block_nums:
            region_first_half_pdf = np.concatenate((region_first_half_pdf, all_blocks[block_num]))


    # looping over second half years
    region_second_half_pdf = np.array([])

    for yr in np.arange(1991, 2021, 2):
        fpath = '/home/rdh0715/NCA_region_bootstrapping/region_' + str(region_num) + '/second_half/' + str(yr) + '_' + str(yr+1) + '/'
        block_file_list = fnmatch.filter(os.listdir(fpath), '*.npy')
        all_blocks = np.array([np.load(fpath + fname) for fname in block_file_list])
        num_blocks = len(block_file_list)  #! Might need to come back to this > should it be the smallest number of block segments?
        rand_block_nums = np.random.choice(np.arange(num_blocks), size = num_blocks, replace = True)
        for block_num in rand_block_nums:
            region_second_half_pdf = np.concatenate((region_second_half_pdf, all_blocks[block_num]))


    # clearing out nans for future calculations
    region_first_half_pdf = region_first_half_pdf[~np.isnan(region_first_half_pdf)]
    region_second_half_pdf = region_second_half_pdf[~np.isnan(region_second_half_pdf)]

    return region_first_half_pdf, region_second_half_pdf


def pdf_moments_diff(bootstrap_num, region_num, confidence_interval=0.05):
    ...
    for i in np.arange(0, bootstrap_num, 1):
        if i % 10 == 0:
            print('region ' +str(region_num) + ': ' + str(i) + ' at ' + str(time.time() - start) + ' seconds')
        if i == (bootstrap_num - 1):
            print('region ' + str(region_num) + ': final loop at ' + str(time.time() - start) + ' seconds')
        pdf1, pdf2 = bootstrap_pdf_creator(region_num)
    ...
    np.save('/home/rdh0715/block_bootstrap_results_NCA/per_99/per_99_region_' + str(region_num) + '.npy', pdf1_99)
    ...
    np.save('/home/rdh0715/block_bootstrap_results_NCA/pdf_shift/pdf_shift_region_' + str(region_num) + '.npy', pdf_shift_quantiles)
    ...
    np.save('/home/rdh0715/block_bootstrap_results_NCA/pdf_shift/pdf_shift_percentiles_region_' + str(region_num) + '.npy', pdf_shift_percentiles_quantiles)
    ...
    np.save('/home/rdh0715/block_bootstrap_results_NCA/pdf_shift/pdf_shift_percentiles_99_region_' + str(region_num) + '.npy', pdf_shift_percentiles_99_quantiles)
...
def wrapper(region):
    ...
    pdf_1_mean[region - 1], pdf_1_median[region - 1], pdf_1_std[region - 1], pdf_1_skew[region - 1], pdf_1_kurt[
        region - 1], \
    mean_diff_low[region - 1], mean_diff_mid[region - 1], mean_diff_high[region - 1], median_diff_low[region - 1], \
    median_diff_mid[region - 1], median_diff_high[region - 1], std_diff_low[region - 1], std_diff_mid[region - 1], \
    std_diff_high[region - 1], skew_diff_low[region - 1], skew_diff_mid[region - 1], skew_diff_high[region - 1], \
    kurt_diff_low[region - 1], kurt_diff_mid[region - 1], kurt_diff_high[region - 1] = pdf_moments_diff(bootstrap_num, region,
                                                                                                        0.05)

    # creating pandas dataframe to store results
    pdf_results = pd.DataFrame(np.arange(1, 11, 1), columns=['regions'])
    ...
    pdf_results.to_csv(
        '/home/rdh0715/block_bootstrap_results_NCA/results/bootstrapped_daily_precipitation_pdf_moments_region_' + str(
            region) + '.csv')
