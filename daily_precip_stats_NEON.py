        ...
        if (len(first_half_qual_years) >= pdf_period_threshold) & (len(second_half_qual_years) >= pdf_period_threshold):
            ...
            for yr in np.arange(1951, 1980, 2):
                if (yr in first_half_qual_years) & ((yr+1) in first_half_qual_years):
                    ...
                    np.save('/home/rdh0715/domain_bootstrapping/domain_' + str(domain) + '/first_half/' + yr_str + '/' + station + '_' + yr_str + '.npy',block_bootstrap_precip)
                else:
                    continue
            ...
            for yr in np.arange(1991, 2020, 2):
                if (yr in second_half_qual_years) & ((yr+1) in second_half_qual_years):
                    ...
                    block_bootstrap_precip = data['PRCP_QUAL'][data['DATE'].dt.year.isin(block_years)]
                    yr_str = str(yr) + '_' + str(yr+1)
                    np.save('/home/rdh0715/domain_bootstrapping/domain_' + str(domain) + '/second_half/' + yr_str + '/' + station + '_' + yr_str + '.npy',block_bootstrap_precip)
                else:
                    continue
...
#%% Printing Test Results
    # np.save('/home/rdh0715/domain_' + str(domain) + '_daily_first_half_pdf_90_27.npy', domain_precip_first_half_pdf)
    # print(np.shape(domain_precip_first_half_pdf))
    # np.save('/home/rdh0715/domain_' + str(domain) + '_daily_second_half_pdf_90_27.npy', domain_precip_second_half_pdf)
    # print(np.shape(domain_precip_second_half_pdf))
    # np.save('/home/rdh0715/domain_' + str(domain) + '_daily_both_first_half_pdf_90_27.npy', domain_precip_both_first_half_pdf)
    # print(np.shape(domain_precip_both_first_half_pdf))
    # np.save('/home/rdh0715/domain_' + str(domain) + '_daily_both_second_half_pdf_90_27.npy', domain_precip_both_second_half_pdf)
...
results_df.to_csv('/home/rdh0715/NEON_domain_daily_precip_stats_90_27.csv')  # saving summary statistics for each domain
